...
function App() {
  ...
  const handlePrevious = () => {
    setCurrentId((prevId) => (prevId === 1 ? 4 : prevId - 1));
  };

  const handleNext = () => {
    setCurrentId((prevId) => (prevId === 4 ? 1 : prevId + 1));
  };
  ...
  const person = ReviewData[currentId - 1];
    ...
    <div className='root'>
      <main>
        <article className='review'>
          ...
          <button className='btn btn-hipster' onClick={handleSurpriseMe}>Surprise Me</button>
        </article>
      </main>
    </div>
  );
